...
df_join_tag = df_result.join(df_tags, df_result.restaurant_id==df_tags.safegraph_place_id, 'inner')\
            .withColumnRenamed('col','restaurant_cuisine').withColumnRenamed('safegraph_place_id','temp_id')
df_join_tag_1 = df_join_tag.join(df_tags, df_join_tag.competitor_id == df_tags.safegraph_place_id, 'inner')\
            .withColumnRenamed('col','competitor_cuisine')
df_output = df_join_tag_1.where(col('restaurant_cuisine') == col('competitor_cuisine'))\
            .select('restaurant_id','competitor_id','restaurant_cuisine').sort(col('restaurant_id'))
